...
void RestrApp::run() {
    ...
    menuOption.on_change = [&] () { switchCollection(selected, menuOption, documentEntries); };
    ...
    auto uiRenderer = ftxui::Renderer(mainComponent, [&] {
        ...
        return ftxui::vbox({
                ftxui::hbox({ftxui::vbox({ ftxui::text("API Collections"), menuWin | ftxui::border | ftxui::flex}), ftxui::vbox({ftxui::text("API Collections"), docMenuWin | ftxui::border | ftxui::flex}) | ftxui::flex }) | ftxui::flex,
        });
    });
